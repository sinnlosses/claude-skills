...
- `done`・`dropped` も同じ場所に残す。**タスクのアーカイブの段は無くなる**
  ...
  残し、書き足さない）。振り返りが済んだものは `task prune`（5.10）で `git rm` する（移す先は
  作らない。本文は git の履歴から読む。2026-09-24 に足した）
...
- 依存の解決: `done` と `dropped` は解決済み。`hold` と `todo` は未解決。**タスクファイルにも
  `docs/history/tasks.md` にも無い ID は解決済み**（旧アーカイブ由来と、`task prune` で消したもの）
...
| 3 | `INVALID` | データの不備（タスクファイルが読めない、移行途中、`- ブランチ:` が読めない、`--check` の重複、`prune` の基準点がリポジトリに無い） | 理由をそのまま報告して止まる。直しに行かない |
...
対象は `HEAD` の版で `done`・`dropped`、台帳に印が無く、次のどちらかに当たるもの:

- `reviewed`: `## 結果` に `- 振り返り:` の行がある（`/next-task` の中の1件ごとの振り返り済み）
- `retrospect`: `develop/retrospective.md` の「最後に振り返ったコミット」の版で既に `done`・`dropped`
  （そうしたコミットが基準点の祖先にある＝手で呼ぶまとめての振り返りが読み終えた）。記録ファイルか
...
ぶんは件名にタスクIDを置かない別の1コミットにし、同じ `ship` で送る。消しても依存の解決（4.1）と
採番（台帳の `last-id`）は変わらず、振り返りの材料は `material.py` が最後の版から読む。
...
2. 手順
   ...
   7. `task done T-xxx --result-file -` → 作業とタスクファイルを1コミット（`git add -A` を
      使わず触ったファイルを個別に）。続けて `task prune` → `PRUNED` なら消したぶんを別の1コミット（5.10）
