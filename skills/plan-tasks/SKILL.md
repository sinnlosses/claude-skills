---
name: plan-tasks
description: "develop/direction.md に書かれたユーザーからの指示（と、承認を得たエージェントのドラフト、## 積み残し）を task new で develop/task/ のタスクに分解して登録し、指示メモを docs/history/direction.md へ移して main へ送る。ユーザーが「指示をタスクにして」「direction.md を処理して」と言ったとき、またはセッション開始時に develop/direction.md に未対応の指示があったときに使う。旧形式（develop/tasks.json）のプロジェクトでは移行の案内を出して止まる。"
---
...
ときに無人でこの手順を読む場合で、そのとき扱ってよいのは `## ユーザーから`（ファイル入口）と
`## 積み残し` だけ（以下「無人のとき」）。
...
1. **読む**: `task status` で既存の一覧を見る（本文は読まない。重なりそうな1件があるときだけ
   ...
   | `## 積み残し` | 分解と承認は済んでいる。手順2（確かめる）だけやり直して手順5へ |
...
3. **分解する**: 1項目＝1タスクとは限らない。方針決めが要るものは前段のタスクに切り出して
   `dependencies` で後ろに置く。1タスクは1コミットで説明が付く大きさに。既存タスクと重なるなら
...
5. **登録する**: 依存の先にあるものから1件ずつ `task new`（`CREATED` の ID を後ろの `--deps` に使う）。
   ...
   | --- | --- |
   | `CREATED` | 次の1件へ |
   | `CAP` | 上限（`READY` が並列数 × 2）。**残り全部**（その ID に依存するはずだったものも）を `develop/direction.md` の `## 積み残し` に、ID を振らずに正典「指示メモ」の形で書く |
   | `LOCKED` | 採番の錠が取れない。行に `rmdir` が添えてあれば古い錠なので、その旨を報告して止まる（壊さない） |
   | 終了コード2 | 本文の節の欠け・禁止の節。直して打ち直す |

   `## 積み残し` から繰り上げた項目は、登録できたら節から消す。古くなって要らない項目は理由を
   添えて消す。
...
6. **指示メモを移す**: `docs/history/direction.md` の先頭に日付見出し（`## YYYY-MM-DD`）で、
   ...
   **当時の記述のまま**移して節を見出しだけに戻し、`## エージェントのドラフト` は承認を得た項目だけを
   出典1行つきで移して未承認は残す。会話入口は発言を生の言い回しのまま書く。`## 積み残し` からの
   繰り上げは、元の指示がすでに履歴にあるので対応表の行だけ足す。
...
- **項目・発言 → タスクID／`## 積み残し` の対応表**（全項目。ドラフト由来は承認の発言も添える。
...
- `CAP` で積み残しに回した件数と、`task status` の `ready` 行
