...
3. **選ぶ**: `着手可否` が `READY` の行から1件。**`/loop` から回っているときは `loopable: N` を
   ...
   1. `## 積み残し` に項目がある → `plan-tasks` スキルを読み、その手順で**積み残しの繰り上げ**だけを
      行う（無人でもよい。正典「指示メモ」）
   2. `## ユーザーから` に中身がある → `plan-tasks` スキルの手順を**ファイル入口の分だけ**行う
      （`/loop` からでもよい。会話入口と `## エージェントのドラフト` には触らない）
   3. どちらかで登録したら `task status` を取り直してこの手順3をやり直す。それでも無ければ、
...
- `task status` を取り直した `ready`（READY 件数/上限）・`counts` の `hold`・`backlog` の件数と、
  `todo_loopable` の `N` の件数（0件なら「全タスク完了」と明言する）
- 手順3で繰り上げ・タスク化したなら、登録したタスクID（そのコミットが完了のコミットとは別であること）
