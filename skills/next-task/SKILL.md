...
1. **見渡す**: 一覧と判定はスクリプトが出す。**`develop/tasks.json` を Read ツールで開いたり
   ...
   `## ユーザーから` に中身があれば**未タスク化の指示が残っている**。分解は方針決めを含むので
   このスキルがその場で代行することはしないが、`READY` なタスクがあるならそちらを止めない。
...
2. **選ぶ**: まず手順1のTSVに `status` が `doing` の行が無いか見る。あれば**前回セッションの
   ...
   手順1で見た `## ユーザーから` の中身の有無で報告を分けて終了する（`## エージェントの
   ドラフト` だけに中身がある場合は「中身が無い」側で扱う。正典「指示メモ」）:
   - `## ユーザーから` に中身がある → 「進められるタスクが無く、未タスク化の指示がある →
     `/plan-tasks` が先」と報告する
   - `## ユーザーから` に中身が無い → 「進められるタスクが無い」とだけ報告する
