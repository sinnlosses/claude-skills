...
1. **見渡す**: 一覧と判定はスクリプトが出す。**`develop/tasks.json` を Read ツールで開いたり
   ...
   指示が残っている**。分解は方針決めを含むので、このスキルがその場で代行することはしないが、
   `READY` なタスクがあるならそちらを止めない。中身がある場合は行数
   （`grep -v '^#' develop/direction.md | grep -v '^\s*$' | wc -l`）を数え、完了報告に添える
   （手順2・「完了報告のフォーマット」で使う）。
...
2. **選ぶ**: まず手順1のTSVに `status` が `doing` の行が無いか見る。あれば**前回セッションの
   ...
   手順1で見た `develop/direction.md` の中身の有無で報告を分けて終了する:
   - 中身がある → 「進められるタスクが無く、未タスク化の指示がある → `/plan-tasks` が先」と報告する
   - 中身が無い → 「進められるタスクが無い」とだけ報告する

   どちらも `/loop` 側はこれを「続行不要」の合図として扱う。
...
8. **マージ**: 作業ブランチをデフォルトブランチへ **fast-forward** で取り込む
   （`git switch main && git merge --ff-only feature/T-XXX`）。成功したら作業ブランチを
   削除する（`git branch -d feature/T-XXX`）。

   **fast-forward できない場合**（main がブランチの分岐後に進んでいる）は、作業ブランチを
   `git rebase main` してから `--ff-only` を再試行する。rebase がコンフリクトなく終われば
   そのまま ff で取り込める。**rebase 中にコンフリクトが起きたら、黙って `--no-ff` に
   落とさず**、コンフリクトしたファイルを報告してユーザーに預けて終了する
   （`resolving-merge-conflicts` スキルで解消できる。`/loop` はこれを「続行不要」の合図として
   扱う）。

   検証コマンドは通常は再実行しない（手順6で通した結果のまま ff できるため、main の内容は
   作業ブランチの到達点と同一になる）。ただし rebase を挟んでコミットの中身が変わった場合は、
   ff で取り込む前にもう一度**検証コマンド**を通す（正典「ファイル配置と CLAUDE.md」の
   「ブランチ運用」）。
...
- 未タスク化の指示（`develop/direction.md`）の有無。あれば行数
